...
class Order(Base):
    ...
    side: Mapped[OrderSide] = mapped_column(sqlalchemy.Enum(OrderSide), nullable=False)
    type: Mapped[OrderType] = mapped_column(sqlalchemy.Enum(OrderType), nullable=False)
    ...
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.now(timezone.utc), onupdate=func.now(), nullable=False)
...
class Trade(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    buy_order_id: Mapped[str] = mapped_column(ForeignKey("orders.id", ondelete="CASCADE"), nullable=False, index=True)
    sell_order_id: Mapped[str] = mapped_column(ForeignKey("orders.id", ondelete="CASCADE"), nullable=False, index=True)
