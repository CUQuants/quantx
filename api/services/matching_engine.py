...
class MarketDataSnapshot:
    ...
    def remove_order(self, price: float, quantity: int, side: OrderSide) -> None:
        ...
        if price not in book:
            logger.warning(f"Price {price} not found in {side.value} book for {self.ticker}")
            return
        
        book[price] -= quantity
        
        if side == OrderSide.BUY:
            self.total_bid_quantity = max(0, self.total_bid_quantity - quantity)
        else:
            self.total_ask_quantity = max(0, self.total_ask_quantity - quantity)
...
class MatchingEngine(BaseService):
    ...
    async def _cross(self, ticker: str, correlation_id: str) -> None:
        ...
        while True:
            ...
            market_data.remove_order(ask_order.price, trade_qty, OrderSide.SELL)
            market_data.update_last_trade(trade_price, trade_qty)
        ...
        for ticker in self.tickers:
            ...
            if book_stats[ticker]["best_bid"] and book_stats[ticker]["best_ask"]:
                book_stats[ticker]["spread"] = (
                    book_stats[ticker]["best_ask"] - book_stats[ticker]["best_bid"]
                )
